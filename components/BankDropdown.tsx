...
export const BankDropdown = ({ accounts = [], setValue, otherStyles }: BankDropdownProps) => {
  ...
  return (
    <Select defaultValue={selected.id} onValueChange={value => handleSelectBank(value)}>
      ...
      <SelectContent className={`w-full bg-[#101727] md:w-[300px] ${otherStyles}`} align="end">
        <SelectGroup>
          ...
          {accounts.map((account: Account) => (
            <SelectItem
              ...
              className="cursor-pointer border-t border-[#344054]"
            >
              <div className="flex flex-col ">
                <p className="text-16 font-medium">{account.name}</p>
                <p className="text-14 font-medium text-[#9309fe]">{formatAmount(account.currentBalance)}</p>
              </div>
            </SelectItem>
          ))}
        </SelectGroup>
